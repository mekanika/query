...
exports.or = function( field, val ) {
  ...
  this.qo.match[ this.qo.match.length-1 ].type = 'or';
  return this;
};
...
each( operators, function( operator ) {
  ...
  exports[ operator ] = function( condition ) {
    ...
    if (operator === 'not') operator = 'neq';

    // Enforce array entries for `in, nin, all`
    if ( /in$|^all/.test( operator ) ) {
      if ( !(condition instanceof Array) )
        throw new Error('Operator "'+operator+'" requires condition as array');
    }

    // Get the most recent 'where' constraint
    var where = this.qo.match[ this.qo.match.length-1 ];
