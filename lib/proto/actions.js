...
exports.create = function( payload, cb ) {
  ...
  if (payload) {
    payload instanceof Array
      ? this.content = this.content.concat( payload )
...
exports.save = function( payload, cb ) {
  ...
  while (payload.length)
    this.content.push( payload.shift() );
...
exports.update = function( conditions, update, cb ) {
  ...
  if (update) this.content.push( update );
...
exports.remove = function( id, cb ) {
  ...
  if (id) {
    ...
    // Or apply ids as identifiers to the query
      ...
      ? this.identifiers = this.identifiers.concat( id )
...
exports.done = function( cb ) {
  ...
  if (!this.resource || !this.action)
    return cb && _wrap.call( this, cb )( 'Must set `from` and `action`');

  // Return the query and an error message if no adapter
  if (!this.adapter)
    return cb && _wrap.call( this, cb )( 'No adapter to query');
  ...
  var pre = _join( hooks.pre.all, hooks.pre[ this.action ] );
  var self = this;
  // Do we have middleware to run on this event
  if ( pre && pre.length )
    pre.map( function(f){ f( self ); } );


  // -- Post action middleware
  var post = _join( hooks.post.all, hooks.post[ this.action ] );
  if ( post && post.length ) {
    var _cb = cb;
    cb = exports.posthook( post, _cb, self );
  }

  // Wrap the final callback to:
  // - 1. Add this query to the arguments stack
  // - 2. Force ASYNC execution (Do not release źálġő)
  //      - http://blog.izs.me/post/59142742143/designing-apis-for-asynchrony
  this.adapter.exec( this, _wrap.call(this, cb) );
